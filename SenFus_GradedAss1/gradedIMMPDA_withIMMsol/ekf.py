...
class EKF:
    ...
    def innovation(self,
        ...
        # reuse the above functions for the innovation and its covariance
    ...
    def update(self,
        ...
        H = self.sensor_model.H(x, sensor_state=sensor_state)
        W = P@la.solve(S, H).T  # the kalman gain, Hint: la.solve, la.inv

        x_upd = x + W@v  #the mean update
        P_upd = (np.eye(np.size(P[0])) - W@H) @ P  #the covariance update
    ...
    def NEES(cls,
        ...
        #NEES = x_diff.T@np.linalg.inv(P)@x_diff
    ...
    def loglikelihood(self,
        ...
        (sign, S_logdet) = np.linalg.slogdet(S)
        # log likelihood, Hint: log(N(v, S))) -> NIS, la.slogdet.
        ll = -0.5*(np.log(2*math.pi) + S_logdet + self.NIS(z,ekfstate))
    ...
    def estimate_sequence(
        ...
        for k, (zk, Tsk, ssk) in enumerate(zip(Z, Ts_arr, sensor_state_seq)):
            if k == 0:
                ...
                ekfpred_list[k] = self.predict(ekfupd_list[k-1], Tsk)

            ekfupd_list[k] = self.update(zk, ekfpred_list[k], sensor_state=ssk)
