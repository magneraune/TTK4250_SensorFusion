...
class IMM(Generic[MT]):
    ...
    def loglikelihood(
        ...
        *,
        ...
        ll = logsumexp(mode_conditioned_ll + immstate.weights)  # weighted average of likelihoods (not log!)
    ...
    def reduce_mixture(
        ...
        for s in range(len(self.filters)):
            mixture = MixtureParameters(
                mode_conditioned_component_prob[s], np.array([c.components for c in immstate_mixture.components])[:,s]
            )
            mode_states.append(self.filters[s].reduce_mixture(mixture))
    ...
    def gate(
        ...
        mode_gated: List[bool] = [
            #fs.gate(z, immstate.components, gate_size_square, sensor_state=sensor_state) for fs in self.filters
            self.filters[0].gate(z, c, gate_size_square, sensor_state=sensor_state) for c in immstate.components
        ]
